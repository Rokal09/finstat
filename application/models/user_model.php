...
class User_model extends CI_Model
{
	...
	public function create($userName,$firstName,$lastName,$password,$email,$contact,$address,$city,$pincode,$dob,$accesslevel,$status)
	{
		$data  = array(
			'username' => $userName,
			'firstname' => $firstName,
    ...
    public function editusersubmit($id,$userName,$firstName,$lastName,$password,$email,$contact,$address,$city,$pincode,$dob,$accesslevel,$status)
	{
		if(empty($password))
        {
             $data  = array(
...
INNER JOIN  `accesslevel` ON  `user`.`accesslevel` =  `accesslevel`.`id` WHERE `user`.`id`='$id'");
	...
	{
		//$user = $this->session->userdata('accesslevel');
		$query="SELECT `id`, `userName`, `firstname`, `lastname`, `password`, `email`, `contact`, `address`, `city`, `pincode`, `dob`, `accesslevel`, `timestamp`, `status` FROM `user` WHERE `accesslevel`='3'";
		$query=$this->db->query($query)->result();
		return $query;
	}
    
    public function createfacultymanagement($firstname,$lastname,$dob,$password,$accesslevel,$email,$contact,$status,$facebookuserid,$website,$description,$address,$city,$pincode)
	{
		$data  = array(
			'firstname' => $firstname,
			'lastname' => $lastname,
    ...
    public function editfacultymanagement($id,$fname,$lname,$dob,$password,$accesslevel,$contact,$status,$facebookuserid,$website,$description,$address,$city,$pincode)
	{
		$data  = array(
			'firstname' => $fname,
			'lastname' => $lname,
		...
		if($query)
		{
			$this->saveuserlog($id,'Faculty Management Details Edited');
		}
		return 1;
	}
     ...
     {
      $query= $this->db->query("SELECT `id`,`name`,`userid` FROM `timesheet` WHERE `userid`=
$id and `paymentid` =0")->result();
     ...
     function makepayment($id)
      {
         //$this->db->where('id', $id);
         $query= $this->db->query("SELECT count(`id`) as count,SEC_TO_TIME(SUM(TIME_TO_SEC(timediff(`timeout`, `timein`)))) as `totalhours`,`userid`, `centerid`, `activity`, `date`, `timein`, `timeout`, `remark`, `expence`, `subjectid`, `chapterid` FROM `timesheet` WHERE `userid`=
$id and `activity`='Lecture' AND `paymentid` =0")->result();
//        if($query->num_rows > 0)
//        {
//            return $query->result();
//        }
